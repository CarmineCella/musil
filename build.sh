...
#   ./build.sh musil           build just one target (musil, musil-ide, musil-docs)
...
#   ./build.sh --ide           build, then open the IDE
#   ./build.sh --no-fltk       language and libraries only, no dependency (no plot, no controls, no IDE),
#                              into build-nofltk/
...
BUILD_DIR="build"; TYPE="Release"; TARGET=""; RUN=""; TEST=""; IDE=""; FLTK="ON"; CLEAN=""; DOCS=""
args=("$@")
for ((i=0; i<${#args[@]}; i++)); do
    arg="${args[$i]}"
    case "$arg" in
        ...
        --ide)       IDE=1 ;;
        --docs)      DOCS=1 ;;
        --no-fltk)   FLTK="OFF"; BUILD_DIR="build-nofltk" ;;
...
echo "==> Configuring ($TYPE, FLTK $FLTK) into $BUILD_DIR"
cmake -B "$BUILD_DIR" -DCMAKE_BUILD_TYPE="$TYPE" -DMUSIL_FLTK="$FLTK"
...
if [[ -n "$IDE" ]]; then
    [[ "$FLTK" == "ON" ]] || { echo "--ide needs FLTK (drop --no-fltk)" >&2; exit 1; }
