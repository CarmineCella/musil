...
static void submit(const std::string& code, const std::string& name);
...
static void quit_cb(Fl_Widget*, void*) { if (!ok_to_discard()) return; S.quit = true; if (S.busy) S.stop = true; S.cv.notify_all(); win->hide(); }   // a running program is interrupted too, else the exit waits for it
