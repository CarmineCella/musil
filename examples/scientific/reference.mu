...
section("0. ABOUT THIS FILE")
print "This reference loads stdlib.mu."
print "Core builtins are always available."
print "Some helpers below (range, split, join, map, filter, reduce, etc.) come from stdlib.mu."

# ───────────────────────────────────────────────────────────────────────────────
#  1. TYPES
#  Five runtime types: number, vector, string, array, proc
...
# ───────────────────────────────────────────────────────────────────────────────
#  3. STRINGS
#  Immutable. + concatenates. If either side is a string, the other operand is
#  stringified. Sub-string, search, case, split.
# ───────────────────────────────────────────────────────────────────────────────
section("3. STRINGS")
...
print "sub(0,5)    = " sub(greeting, 0, 5)
...
# ───────────────────────────────────────────────────────────────────────────────
#  7. PROCS (NAMED)
#  proc name (params) { body } — use explicit return for the result.
# ───────────────────────────────────────────────────────────────────────────────
...
print "ops[0](10,3) = " ops[0](10, 3)
print "ops[1](10,3) = " ops[1](10, 3)
print "ops[2](10,3) = " ops[2](10, 3)
...
var v1 = vec(1, 2, 3, 4, 5)
var v2 = linspace(0, 1, 5)
var v3 = zeros(4)
var v4 = ones(3)
print "vec(1..5):       " v1
...
# Vector arithmetic requires equal lengths unless one side is a scalar.
print "vec(1,2,3) + vec(4,5,6) = " (vec(1,2,3) + vec(4,5,6))

print "sqrt(v1):  " sqrt(v1)
print "sin(v2*PI):" sin(v2 * PI)
...
print "sum(v1)    = " sum(v1)
print "all(v1>0)  = " all(v1 > 0)
print "any(v1>4)  = " any(v1 > 4)
...
print "iv[-1] = " iv[-1]
iv[1] = 99
print "after iv[1]=99: " iv

var ia = ["a", "b", "c"]
print "ia[0]  = " ia[0]
print "ia[-1] = " ia[-1]
ia[1] = "B"
print "after ia[1]=\"B\": " ia

# ───────────────────────────────────────────────────────────────────────────────
#  9B. VALUE VS REFERENCE
# ───────────────────────────────────────────────────────────────────────────────
section("9B. VALUE VS REFERENCE")

var vv1 = vec(1, 2, 3)
var vv2 = vv1
vv2[0] = 99
print "vectors copy on assignment: " vv1 " / " vv2

var aa1 = [1, 2, 3]
var aa2 = aa1
aa2[0] = 99
print "arrays share on assignment: " aa1 " / " aa2
...
push(fruits, "date")
push(fruits, "elderberry", "fig")
print "after push: " fruits

var popped = pop(fruits)
print "popped: " popped "  remaining: " fruits

insert(fruits, 1, "avocado")
print "after insert at 1: " fruits

var removed2 = remove(fruits, 0)
print "removed: " removed2 "  remaining: " fruits

var arr1 = [10, 20, 30]
var arr2 = arr1
arr2[0] = 99
print "arr1[0] after modifying arr2: " arr1[0]

var arr3 = copy(arr1)
arr3[0] = 777
print "arr1[0] after modifying copy: " arr1[0]

proc push_ten (a7) { push(a7, 10) }
var mylist = [1, 2, 3]
push_ten(mylist)
print "after push_ten: " mylist
...
var matrix_arr = [[1,2,3],[4,5,6],[7,8,9]]
print "matrix_arr[1][2] = " matrix_arr[1][2]
matrix_arr[1][2] = 99
print "after assign: " matrix_arr[1]
...
# ───────────────────────────────────────────────────────────────────────────────
#  10A. MATRICES
#  Matrices are arrays of numeric row vectors.
#  Use explicit scientific builtins rather than + - * /.
# ───────────────────────────────────────────────────────────────────────────────
section("10A. MATRICES")
...
print "matadd(M1, M2):"
matdisp(matadd(M1, M2))

print "matsub(M2, M1):"
matdisp(matsub(M2, M1))

print "hadamard(M1, M2):"
matdisp(hadamard(M1, M2))

print "matmul(M1, M2):"
matdisp(matmul(M1, M2))

print "matscale(M1, 10):"
matdisp(matscale(M1, 10))

print "matshift(M1, 100):"
matdisp(matshift(M1, 100))

var vx2 = vec(10, 20)
print "matvec(M1, vx2) = " matvec(M1, vx2)
print "vecmat(vx2, M1) = " vecmat(vx2, M1)

# Important:
# + - * / are for scalars and vectors.
# For matrices use matadd, matsub, matmul, hadamard, matscale, matshift.
...
# ───────────────────────────────────────────────────────────────────────────────
#  17A. ASSERTIONS AND DEBUGGING
# ───────────────────────────────────────────────────────────────────────────────
section("17A. ASSERTIONS AND DEBUGGING")

assert_eq(1 + 1, 2, "basic arithmetic")
assert_near(sqrt(2) * sqrt(2), 2, 0.000001, "sqrt consistency")
print "keys() = " keys()
...
var c_major_offsets = vec(0, 2, 4, 5, 7, 9, 11, 12)
var c_major_midi = c_major_offsets + 60
var c_major_hz = midi_to_hz(c_major_midi)
