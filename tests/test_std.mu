...
check (equal? (map (range 3) (function (k) (list k (* k k)))) (list (list 0 0) (list 1 1) (list 2 4))) "map on a vector with non-numeric results gives a list"
...
# --- records and lists by a key ---
var rec (record (list 'freq 440 'dur 0.5))
check (equal? rec (list (list 'freq 440) (list 'dur 0.5))) "record: pairs from alternating keys and values"
check (== (get rec 'freq) 440) "get"
check (== (opt rec 'nope 7) 7) "opt: default"
check (== (opt rec "dur" 0) 0) "opt: keys compare with equal?, so a string is not a symbol (use str on both sides to mix them)"
check (contains? (error-of (function () (get rec 'nope))) "no key") "get: missing key is an error"
check (has? rec 'dur) "has?"
check (not (has? rec 'amp)) "has?: absent"
put! rec 'dur 1
check (== (get rec 'dur) 1) "put!: replaces in place"
put! rec 'amp 0.3
check (== (length rec) 3) "put!: adds"
var rec2 (put rec 'amp 0.9)
check (and (== (get rec2 'amp) 0.9) (== (get rec 'amp) 0.3)) "put: a copy with the key set"
check (equal? (keys rec) (list 'freq 'dur 'amp)) "keys"
check (equal? (values rec) (list 440 1 0.3)) "values"
check (equal? (sort-by (list "pear" "fig" "apple") length) (list "fig" "pear" "apple")) "sort-by: by a key"
check (equal? (sort-by (list (list 2 "b") (list 1 "a") (list 2 "c")) head) (list (list 1 "a") (list 2 "b") (list 2 "c"))) "sort-by: stable"
check (equal? (sort-by (list) identity) (list)) "sort-by: empty"
check (equal? (group-by (list 1 2 3 4 5 6) odd?) (list (list 1 (list 1 3 5)) (list 0 (list 2 4 6)))) "group-by"
check (== (find-first (list 1 4 9) (function (x) (> x 3))) 4) "find-first"
check (equal? (type (find-first (list 1 2) (function (x) (> x 3)))) "nil") "find-first: none is nil"
check (equal? (reject (list 1 2 3 4) odd?) (list 2 4)) "reject"
check (equal? (min-by (list "aaa" "b" "cc") length) "b") "min-by"
check (equal? (max-by (list "aaa" "b" "cc") length) "aaa") "max-by"
check (== (sum-by (list 1 2 3) (function (x) (* x x))) 14) "sum-by"
check (equal? (partition (list 1 2 3 4) odd?) (list (list 1 3) (list 2 4))) "partition"
check (equal? (remove-at (list 1 2 3) 1) (list 1 3)) "remove-at"
check (equal? (insert-at (list 1 3) 1 2) (list 1 2 3)) "insert-at"
report "test_std"
